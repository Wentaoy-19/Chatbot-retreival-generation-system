...
class GPT2HuggingFace(FrameworkCommand):
    ...
    def generate_and_download_framework(
        ...
        model = None
        tfm_config = OPTConfig(use_cache=cache_variant)

        if not os.path.exists(pytorch_model_dir):
            # Generate the pre-trained weights
            model = OPTForCausalLM(tfm_config).from_pretrained(metadata.variant)
            ...
            model = OPTForCausalLM(tfm_config).from_pretrained(pytorch_model_dir)
    ...
    def execute_inference(
        ...
        config = OPTConfig(use_cache=metadata.other.kv_cache)
        gpt2_model = OPTForCausalLM(config).from_pretrained(gpt2_torch_fpath)
